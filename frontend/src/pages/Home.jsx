...
const Home = () => {
    ...
    return (
        <div className="mt-8 grid gap-4 sm:gap-6 lg:gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mx-20">
            {loading ? (
                // Loading indicator
                <div className="text-center col-span-1 sm:col-span-2 lg:col-span-3 xl:col-span-4">
                    ...
                    <p className="text-md text-primary mt-2">Loading places...</p>
                </div>
            ) : (
                places.length > 0 && places.map(place => (
                    <Link to={`/place/${place.id}`} key={place.id}>
                        <div className="bg-gray-500 mb-2 rounded-2xl overflow-hidden">
                            {place.photos?.[0] && (
                                <Image className="rounded-2xl object-cover w-full h-40 sm:h-48 lg:h-56" src={place.photos?.[0]} alt="" />
                            )}
                        </div>
                        <h2 className="font-medium text-md tracking-tight sm:text-md lg:text-lg xl:font-semibold xl:text-xl">{place.address}</h2>
                        <h3 className="text-xs sm:text-sm text-gray-500 leading-4">{place.title}</h3>
